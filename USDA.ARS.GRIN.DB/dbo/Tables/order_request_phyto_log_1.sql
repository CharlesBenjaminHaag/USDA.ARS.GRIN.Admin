CREATE TABLE [order_request_phyto_log] (
    ...
    CONSTRAINT [fk_orpl_created] FOREIGN KEY ([created_by]) REFERENCES [cooperator] ([cooperator_id]),
    CONSTRAINT [fk_orpl_inspected_c] FOREIGN KEY ([inspected_by_cooperator_id]) REFERENCES [cooperator] ([cooperator_id]),
    CONSTRAINT [fk_orpl_modified] FOREIGN KEY ([modified_by]) REFERENCES [cooperator] ([cooperator_id]),
    CONSTRAINT [fk_orpl_or] FOREIGN KEY ([order_request_id]) REFERENCES [order_request] ([order_request_id]),
    CONSTRAINT [fk_orpl_owned] FOREIGN KEY ([owned_by]) REFERENCES [cooperator] ([cooperator_id])
);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_orpl_created]
    ON [order_request_phyto_log]([created_by] ASC);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_orpl_inspected_c]
    ON [order_request_phyto_log]([inspected_by_cooperator_id] ASC);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_orpl_modified]
    ON [order_request_phyto_log]([modified_by] ASC);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_orpl_or]
    ON [order_request_phyto_log]([order_request_id] ASC);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_orpl_owned]
    ON [order_request_phyto_log]([owned_by] ASC);


GO
CREATE UNIQUE NONCLUSTERED INDEX [ndx_uniq_orpl]
    ON [order_request_phyto_log]([order_request_id] ASC, [phyto_certificate_type_code] ASC, [phyto_certificate_identifier] ASC);


GO
GRANT DELETE
    ON OBJECT::[order_request_phyto_log] TO [gg_user]
    AS [dbo];
...
GO
GRANT SELECT
    ON OBJECT::[order_request_phyto_log] TO [gg_search]
    AS [dbo];
