CREATE TABLE [ne9_site_inventory] (
    ...
    CONSTRAINT [fk_ne9si_created] FOREIGN KEY ([created_by]) REFERENCES [cooperator] ([cooperator_id]),
    CONSTRAINT [fk_ne9si_i] FOREIGN KEY ([inventory_id]) REFERENCES [inventory] ([inventory_id]),
    CONSTRAINT [fk_ne9si_modified] FOREIGN KEY ([modified_by]) REFERENCES [cooperator] ([cooperator_id]),
    CONSTRAINT [fk_ne9si_owned] FOREIGN KEY ([owned_by]) REFERENCES [cooperator] ([cooperator_id])
);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_ne9si_created]
    ON [ne9_site_inventory]([created_by] ASC);


GO
CREATE UNIQUE NONCLUSTERED INDEX [ndx_fk_ne9si_i]
    ON [ne9_site_inventory]([inventory_id] ASC);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_ne9si_modified]
    ON [ne9_site_inventory]([modified_by] ASC);


GO
CREATE NONCLUSTERED INDEX [ndx_fk_ne9si_owned]
    ON [ne9_site_inventory]([owned_by] ASC);


GO
GRANT DELETE
    ON OBJECT::[ne9_site_inventory] TO [gg_user]
    AS [dbo];
...
GO
GRANT SELECT
    ON OBJECT::[ne9_site_inventory] TO [gg_search]
    AS [dbo];
