...
BEGIN
	...
	SELECT 
		...
		,[owned_by]
	FROM 
		[gringlobal].[dbo].[literature]
	WHERE
		reference_title LIKE '%' + @search_string + '%'
	OR
		abbreviation LIKE '%' + @search_string + '%'
	OR
		standard_abbreviation LIKE '%' + @search_string + '%'
	OR
		editor_author_name LIKE '%' + @search_string + '%'
	OR
		url LIKE '%' + @search_string + '%'
	OR
